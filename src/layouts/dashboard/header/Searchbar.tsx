...
function Searchbar() {
  ...
  const allItems: Option[] = flattenArray(reduceItems).map(option => ({
    group: option.group && option.group.length > 1 ? option.group[0] : option.subheader,
    title: option.title,
    path: option.path,
    indexKey: 'minimal',
    subheader: (option as Option).subheader || '',
  }));
  ...
  return (
    <ClickAwayListener onClickAway={handleClose}>
      <div>
        {!open && (
          <IconButtonAnimate onClick={handleOpen} disableAnimation>
