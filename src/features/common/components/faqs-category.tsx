...
export default function FaqsCategory({ onChangeCategory, selectedCategory }: FaqsCategoryProps) {
...
function CardDesktop({ category, onChangeCategory, isSelected }: CardDesktopProps) {
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        borderRadius: 2,
        bgcolor: 'unset',
        boxShadow: theme => (isSelected ? theme.customShadows.z24 : 'none'),
        cursor: 'pointer',
        textAlign: 'center',
        '&:hover': {
...
function CardMobile({ category, onChangeCategory }: CardDesktopProps) {
  const { label, icon } = category;
