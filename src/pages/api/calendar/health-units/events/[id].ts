...
export default async function eventsRoute(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'GET') {
      const response = await axios.get(`/calendar/health-units/events/${eventId}`, {
    ...
    if (req.method === 'PUT') {
      ...
      const response = await axios.put(
        `/calendar/health-units/events/${eventId}`,
        { ...event },
        {
          // Set authorization header bearer token
          headers: {
    ...
    if (req.method === 'DELETE') {
      const response = await axios.delete(`/calendar/health-units/events/${eventId}`, {
        // Set authorization header bearer token
        headers: {
          'Content-Type': 'application/json',
          'x-access-token': accessToken,
